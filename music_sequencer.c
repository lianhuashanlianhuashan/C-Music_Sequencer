...
BST_Node *newBST_Node(double freq, int bar, double index)
{
	...
	if (new_node==NULL)
	{
		printf("newBST_Node() : Error. Not enough memory to create a new node, returning NULL.\n");
		return NULL;
	}
...
BST_Node *BST_insert(BST_Node *root, BST_Node *new_node)
{
	...
	{
		if (new_node->index+0.0000001<=1.0){
			new_node->index+=0.0000001;
...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
	...
	if (root==NULL)
		return NULL;
	if (root->key-((10.0*bar)+index)<0.0001&&root->key-((10.0*bar)+index)>-0.0001){
		return root;
	}
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
	...
	}else if (root->key<(10.0*bar)+index){
...
void rvs(BST_Node *root, int min_bar, int max_bar)
{    
	if (root!=NULL){
		root->bar=min_bar+max_bar-root->bar;
		root->index=1.0-root->index;
		rvs(root->left, min_bar, max_bar);
		rvs(root->right, min_bar, max_bar);
	}
}

BST_Node *min(BST_Node *right_child_node)
{
    /*
     * This function finds the successor of a node by 
     * searching the right subtree for the node that
...
BST_Node *max(BST_Node *right_child_node)
{
    /*
     * This function finds the successor of a node by 
     * searching the right subtree for the node that
	...
	while (right_child_node->right!=NULL){
...
BST_Node *reverseSong(BST_Node *root)
{
	...
	int max_bar=max(root)->bar;
	int min_bar=min(root)->bar;
	rvs(root, min_bar, max_bar);
	return root;
}
...
BST_Node *deepcopy(BST_Node *root)
{    
	if (root!=NULL){
		BST_Node *new = newBST_Node(root->freq, root->bar, root->index);
		new->left = deepcopy(root->left);
		new->right = deepcopy(root->right);
		return new;
	}
	return NULL;
}
...
void traverse(BST_Node *main, BST_Node *root, int semitones, double time_shift)
{    
	if (root!=NULL){
		int j=0;
		while (note_freq[j]<root->freq){j++;}
		if (j+semitones>=0&&j+semitones<=99&&1.0-root->index<1.0&&1.0-root->index>=0.0){
			BST_Node *new=newBST_Node(note_freq[j + semitones], root->bar, 1.0-root->index);
			main=BST_insert(main, new);
		}
		traverse(main, root->left, semitones, time_shift);
		traverse(main, root->right, semitones, time_shift);
	}
}
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
	...
	BST_Node *tmp=deepcopy(root);
	traverse(root, tmp, semitones, time_shift);      //root is modified
	return root;
